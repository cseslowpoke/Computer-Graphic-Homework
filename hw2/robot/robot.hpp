...
void draw() {
  ...
  if(!is_walk && (abs(torsola.get_walk() - 45 ) > 1 && abs(torsola.get_walk() - 135 ) > 1) ) {
      torsola.walk_forward();torsola.walk_forward();torsola.walk_forward();torsola.walk_forward();torsola.walk_forward();
  }
  torsola.draw();
  glPopMatrix();
}
