...
void draw() {
  ...
  if (walk > 0) {
    for (int i = 0; i < walk / 5 + 1; i++) {
      ...
      if (environment::detect_collision(x + 0.002 * std::sin(angle * PI / 180),
                                        z + 0.002 * std::cos(angle * PI / 180)))
        continue;
      x += 0.002 * std::sin(angle * PI / 180);
      z += 0.002 * std::cos(angle * PI / 180);
    }
    if (walk > is_walk / 60) {
      walk = is_walk / 60;
  ...
  // if(!is_walk && (abs(torsola.get_walk() - 45 ) > 1 && abs(torsola.get_walk()
  // - 135 ) > 1) ) {
  //     torsola.walk_forward();
  // }
  torsola.draw();
