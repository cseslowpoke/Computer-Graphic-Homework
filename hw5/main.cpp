...
void myinit() {
  ...
  // glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_COLOR, r, g, b, 1.0);
...
void make_view(int mode) {
  ...
  glGetFloatv(GL_MODELVIEW_MATRIX, billboardmtx);
}
  ...
  if (x == 4) {
    ...
    if (width > height)
      glOrtho(-40.0 * parl_zoom / 100, 40.0 * parl_zoom / 100,
              ...
              40.0 * (float)height / (float)width * parl_zoom / 100, -200, 200);
    else
      glOrtho(-40.0 * (float)width / (float)height * parl_zoom / 100,
              40.0 * (float)width / (float)height * parl_zoom / 100,
              -40.0 * parl_zoom / 100, 40.0 * parl_zoom / 100, -200, 200);
  }
  glMatrixMode(GL_MODELVIEW);
}
...
void draw_style(PROJECTION_MODE style, int x, int y, int width, int height) {
  ...
  float w0 = billboardmtx[2], w2 = billboardmtx[10];
  float len = sqrt(w0 * w0 + w2 * w2);
  environment::draw(w2/len, -w0/len);
  robot::draw();
}
